...
public class Store {
	...
	public Store(String cStorePath, int cSize, String lStorePath, int lSize) {
		...
		consumeQueueTable = new ConcurrentHashMap<String/* topic */, ConcurrentHashMap<Integer/* queueId */, ConsumeQueue>>();
	}
		...
		ALL: for (MapedFile mapedFile : mapedFiles) {
			...
			while (byteBuffer.hasRemaining()) {
				...
				if (physicOffset != currentPhyOffset) {
					System.out.println(storeTime + " [fetal error] physicOffset != currentPhyOffset. position="
				...
				try {
					...
					if (physicOffset != offsetPy) {
						System.out.println(storeTime + " [fetal error] physicOffset != offsetPy. position=" + position
					...
					if (totalSize != sizePy) {
						System.out.println(storeTime + " [fetal error] totalSize != sizePy. position=" + position
								+ ", msgCount=" + msgCount + ", totalSize=" + totalSize + ", sizePy=" + sizePy);
						errorCount++;
						if (!openAll) {
							success = false;
							break ALL;
			...
			System.out.println("end travel " + mapedFile.getFileName() + ", total msg=" + msgCount + ", error count="
					+ errorCount + ", cost:" + (System.currentTimeMillis() - startTime));
		}
