...
public class NettyRemotingClient extends NettyRemotingAbstract implements RemotingClient {
	...
	@Override
	public void start() {
		defaultEventExecutorGroup = new DefaultEventExecutorGroup(nettyClientConfig.getClientWorkerThreads(), new ThreadFactory() {
			private AtomicInteger threadIndex = new AtomicInteger(0);
			@Override
			public Thread newThread(Runnable r) {
		...
		bootstrap.group(this.eventLoopGroupWorker).channel(NioSocketChannel.class)//
				 ...
				 .handler(new ChannelInitializer<SocketChannel>() {
					 ...
					 public void initChannel(SocketChannel socketChannel) throws Exception {
						 ...
						 socketChannel.pipeline().addLast(defaultEventExecutorGroup, handlers);
					 }
				 });
		...
		if (channel != null && channel.isActive()) {
			...
			if (!channel.isWritable()) {
				throw new RemotingTooMuchRequestException(
						String.format("the channel[%s] is not writable now", channel.toString()));
			}
