...
public abstract class NettyRemotingAbstract {
				...
				try {
					...
					if (event != null && listener != null) {
						switch (event.getType()) {
							case IDLE	  : listener.onChannelIdle(event.getRemoteAddr(), event.getChannel()); break;
							case CLOSE	  : listener.onChannelClose(event.getRemoteAddr(), event.getChannel()); break;
							case CONNECT  : listener.onChannelConnect(event.getRemoteAddr(), event.getChannel()); break;
							case EXCEPTION: listener.onChannelException(event.getRemoteAddr(), event.getChannel()); break;
							default: break;
						}
					}
				} catch (Exception e) {
	...
	public NettyRemotingAbstract(final int permitsOneway, final int permitsAsync) {
		this.semaphoreOneway = new Semaphore(permitsOneway, true);// 256
		this.semaphoreAsync  = new Semaphore(permitsAsync , true);// 64
	}
			...
			Runnable run = new Runnable() {
				...
				public void run() {
					try {
						...
						// TODO: === 获取NettyRequestProcessor
						NettyRequestProcessor nettyRequestProcessor = pair.getObject1();
						// TODO: === 处理请求
						final RemotingCommand response = nettyRequestProcessor.processRequest(ctx, cmd);
						if (rpcHook != null) {
						...
						if (!cmd.isOnewayRPC()) {
							...
							String remark = RemotingHelper.exceptionSimpleDesc(e);
	...
	public void processMessageReceived(ChannelHandlerContext ctx, RemotingCommand msg) throws Exception {
		...
		if (cmd != null) {
			switch (cmd.getType()) {
				case  REQUEST_COMMAND: processRequestCommand( ctx, cmd); break;
				case RESPONSE_COMMAND: processResponseCommand(ctx, cmd); break;
				default: break;
			}
		}
	}
