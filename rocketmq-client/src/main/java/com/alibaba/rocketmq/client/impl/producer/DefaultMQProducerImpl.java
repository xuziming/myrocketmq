...
public class DefaultMQProducerImpl implements MQProducerInner {
	...
	public void registerCheckForbiddenHook(CheckForbiddenHook checkForbiddenHook) {
		...
		logger.info("register a new checkForbiddenHook. hookName={}, allHookSize={}", 
	...
	public void start(final boolean startFactory) throws MQClientException {
		switch (this.serviceState) {
			case CREATE_JUST:
				...
				// TODO: === 初始化MQClientInstance,一个进程只会存在一个MQClientInstance， 
				mqClientFactory = MQClientManager.getInstance().getAndCreateMQClientInstance(defaultMQProducer, rpcHook);
				...
				this.topicPublishInfoTable.put(this.defaultMQProducer.getCreateTopicKey(), new TopicPublishInfo());
				 ...
				 // 启动MQClientInstance
				if (startFactory) {
					mqClientFactory.start();// mQClientFactory === MQClientInstance
				}

				logger.info("the producer [{}] start OK", this.defaultMQProducer.getProducerGroup());
				this.serviceState = ServiceState.RUNNING;// 设置状态为RUNNING
				break;
			case RUNNING:
			case START_FAILED:
			...
			private void processTransactionState(final LocalTransactionState localTransactionState, final String producerGroup, final Throwable exception) {
				...
				switch (localTransactionState) {
					...
					case UNKNOW:
						...
						logger.warn("when broker check, client donot know this transaction state, {}", thisHeader);
						break;
					default:
						break;
				}
	...
	public void updateTopicPublishInfo(final String topic, final TopicPublishInfo info) {
		if (info != null && topic != null) {
			...
			if (prev != null) {
				...
				logger.info("updateTopicPublishInfo prev is not null, " + prev.toString());
			}
		}
	}
	...
	private void makeSureStateOK() throws MQClientException {
		if (this.serviceState != ServiceState.RUNNING) {
			throw new MQClientException("The producer service state not OK, "
				+ serviceState + FAQUrl.suggestTodo(FAQUrl.CLIENT_SERVICE_NOT_OK), null);
		}
	}
	...
	/**
	 * 无论哪种发送方式，最终都是调用的sendDefaultImpl来提交消息
	 */
	private SendResult sendDefaultImpl(Message msg, final CommunicationMode communicationMode, final SendCallback sendCallback, 
		...
		if (topicPublishInfo != null && topicPublishInfo.ok()) {
				...
				if (mqSelected != null) {// 获取Queue成功
					...
					try {
						...
						switch (communicationMode) {
							...
							case SYNC  : if (sendResult.getSendStatus() != SendStatus.SEND_OK) {
						...
						logger.warn("sendKernelImpl exception", e);
			...
			String info = String.format("Send [%d] times, still failed, cost [%d]ms, Topic: %s, BrokersSent: %s", 
				times, (System.currentTimeMillis() - beginTimestamp), msg.getTopic(), Arrays.toString(brokersSent));
		...
		List<String> nameServerAddressList = getMqClientFactory().getMQClientAPIImpl().getNameServerAddressList();
		if (null == nameServerAddressList || nameServerAddressList.isEmpty()) {
	 ...
	 * 发送消息
	 */
	private SendResult sendKernelImpl(final Message msg, final MessageQueue messageQueue,
			final CommunicationMode communicationMode, final SendCallback sendCallback, final long timeout)
					throws MQClientException, RemotingException, MQBrokerException, InterruptedException {
		// broker(master)
		String brokerAddr = mqClientFactory.findBrokerAddressInPublish(messageQueue.getBrokerName());
		if (null == brokerAddr) {
			tryToFindTopicPublishInfo(messageQueue.getTopic());
			brokerAddr = mqClientFactory.findBrokerAddressInPublish(messageQueue.getBrokerName());
		}

		SendMessageContext context = null;
		if (brokerAddr != null) {
			if (defaultMQProducer.isSendMessageWithVIPChannel()) {
			...
			try {
				...
				SendResult sendResult = mqClientFactory.getMQClientAPIImpl().sendMessage(//
						brokerAddr, // 1
						messageQueue.getBrokerName(), // 2
						msg, // 3
						requestHeader, // 4
						timeout, // 5
						communicationMode, // 6
	...
	public void sendOneway(Message msg, MessageQueueSelector selector, Object arg)
				throws MQClientException, RemotingException, InterruptedException {
		try {
			this.sendSelectImpl(msg, selector, arg, CommunicationMode.ONEWAY, null, defaultMQProducer.getSendMsgTimeout());
		} catch (MQBrokerException e) {
			throw new MQClientException("unknow exception", e);
		}
	}
	...
	private void endTransaction(final SendResult sendResult, final LocalTransactionState localTransactionState, 
		...
		final String brokerAddr = mqClientFactory.findBrokerAddressInPublish(sendResult.getMessageQueue().getBrokerName());
		EndTransactionRequestHeader requestHeader = new EndTransactionRequestHeader();
		requestHeader.setTransactionId(transactionId);
		requestHeader.setCommitLogOffset(id.getOffset());
		switch (localTransactionState) {
			case COMMIT_MESSAGE	 : requestHeader.setCommitOrRollback(MessageSysFlag.TransactionCommitType)	; break;
			case ROLLBACK_MESSAGE: requestHeader.setCommitOrRollback(MessageSysFlag.TransactionRollbackType); break;
			case UNKNOW			 : requestHeader.setCommitOrRollback(MessageSysFlag.TransactionNotType)		; break;
			default: break;
		}
		...
		mqClientFactory.getMQClientAPIImpl().endTransactionOneway(brokerAddr, requestHeader, remark, defaultMQProducer.getSendMsgTimeout());
	}
