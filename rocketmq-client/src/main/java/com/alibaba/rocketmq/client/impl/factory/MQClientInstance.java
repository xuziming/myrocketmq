...
public class MQClientInstance {
	...
	private final ConcurrentHashMap<String/* Broker Name */, Map<Long/* brokerId */, String/* address */>> brokerAddrTable = new ConcurrentHashMap<String, Map<Long, String>>();
	...
	public MQClientInstance(ClientConfig clientConfig, int instanceIndex, String clientId, RPCHook rpcHook) {
		...
		this.mqClientApiImpl = new MQClientAPIImpl(nettyClientConfig, clientRemotingProcessor, rpcHook, clientConfig.getUnitName());
		...
		logger.info("created a new client Instance, FactoryIndex: {} ClinetID: {} {} {}, serializeType={}", this.instanceIndex, this.clientId, 
					this.clientConfig, MQVersion.getVersionDesc(MQVersion.CurrentVersion), RemotingCommand.getSerializeTypeConfigInThisServer());
	}
		...
		synchronized (this) {
			switch (this.serviceState) {
				case CREATE_JUST:
					...
					// TODO: === 1、如果NameservAddr为空，尝试从http server获取nameserv的地址
					if (null == this.clientConfig.getNamesrvAddr()) {
						clientConfig.setNamesrvAddr(mqClientApiImpl.fetchNameServerAddr());
					}
					// Start request-response channel
					// TODO: === 2、启动MQClientAPIImpl，初始化NettyClient
					this.mqClientApiImpl.start();
					// Start various schedule tasks
					// TODO: === 3、开启Client的定时任务
					this.startScheduledTask();
					// Start pull service
					// TODO: === 4、开始处理PullRequest
					this.pullMessageService.start();
					// Start rebalance service
					// TODO: === 5、开启负载均衡服务
					this.rebalanceService.start();
					// Start push service
					// TODO: === 6、启动Client内置的producer
	...
	private void cleanOfflineBroker() {
		try {
			if (this.lockNamesrv.tryLock(LockTimeoutMillis, TimeUnit.MILLISECONDS))
				try {
					ConcurrentHashMap<String, Map<Long, String>> updatedTable = new ConcurrentHashMap<String, Map<Long, String>>();

					Iterator<Entry<String, Map<Long, String>>> itBrokerTable = this.brokerAddrTable.entrySet().iterator();
					while (itBrokerTable.hasNext()) {
						Entry<String, Map<Long, String>> entry = itBrokerTable.next();
						String brokerName = entry.getKey();
						Map<Long, String> oneTable = entry.getValue();

						Map<Long, String> cloneAddrTable = new HashMap<Long, String>();
	...
	private void uploadFilterClassToAllFilterServer(final String consumerGroup, final String fullClassName,
		...
		try {
			...
			logger.warn("uploadFilterClassToAllFilterServer Exception, ClassName: {} {}", 
					fullClassName, RemotingHelper.exceptionSimpleDesc(e1));
		}
			...
			while (it.hasNext()) {
				...
				for (final String fsAddr : value) {
					try {
						...
						logger.info("register message class filter to {} OK, ConsumerGroup: {} Topic: {} ClassName: {}",
	...
	private void sendHeartbeatToAllBroker() {
		...
		Iterator<Entry<String, Map<Long, String>>> it = this.brokerAddrTable.entrySet().iterator();
		while (it.hasNext()) {
			Entry<String, Map<Long, String>> entry = it.next();
			String brokerName = entry.getKey();
			Map<Long, String> oneTable = entry.getValue();
			if (oneTable != null) {
				for (Long id : oneTable.keySet()) {
					...
					if (addr != null) {
						...
						try {
							...
							logger.error("send heart beat to broker exception", e);
						}
					}
				}
			}
			...
			if (this.lockNamesrv.tryLock(LockTimeoutMillis, TimeUnit.MILLISECONDS)) {
				try {
					...
					if (isDefault && defaultMQProducer != null) {
						topicRouteData = mqClientApiImpl.getDefaultTopicRouteInfoFromNameServer(defaultMQProducer.getCreateTopicKey(), 1000 * 3);
						if (topicRouteData != null) {
				...
				for (Long id : oneTable.keySet()) {
					...
					if (addr != null) {
						try {
							...
							logger.info("unregister client[Producer: {} Consumer: {}] from broker[{} {} {}] success",
	...
	public void resetOffset(String topic, String group, Map<MessageQueue, Long> offsetTable) {
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				logger.info("[reset-offset] reset offsetTable. topic={}, group={}, mq={}, offset={}",
