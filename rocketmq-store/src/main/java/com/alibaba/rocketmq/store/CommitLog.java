...
public class CommitLog {
	...
	public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
		...
		// 写message时加入同步锁
		synchronized (this) {
			long beginLockTimestamp = this.defaultMessageStore.getSystemClock().now();// 记录lock time

			// Here settings are stored timestamp, in order to ensure an orderly
			// global
			msg.setStoreTimestamp(beginLockTimestamp);

			// 新建一个maped file如果文件不存在或者文件已经写满
			if (null == mapedFile || mapedFile.isFull()) {
				mapedFile = this.mapedFileQueue.getLastMapedFile();// Mark: NewFile may be cause noise
			}

			if (null == mapedFile) {// 文件创建失败，则返回错误
