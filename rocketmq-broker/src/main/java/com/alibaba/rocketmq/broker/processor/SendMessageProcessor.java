...
public class SendMessageProcessor extends AbstractSendMessageProcessor implements NettyRequestProcessor {
    ...
    private RemotingCommand consumerSendMsgBack(final ChannelHandlerContext ctx, final RemotingCommand request) throws RemotingCommandException {
        ...
        SubscriptionGroupConfig subscriptionGroupConfig = brokerController.getSubscriptionGroupManager().findSubscriptionGroupConfig(requestHeader.getGroup());
        ...
        if (!PermName.isWriteable(this.brokerController.getBrokerConfig().getBrokerPermission())) {
            ...
            response.setRemark("the broker[" + brokerController.getBrokerConfig().getBrokerIP1() + "] sending message is forbidden");
            return response;
        }
        ...
        if (msgExt.getReconsumeTimes() >= subscriptionGroupConfig.getRetryMaxTimes() || delayLevel < 0) {
            ...
            topicConfig = brokerController.getTopicConfigManager().createTopicInSendMessageBackMethod(newTopic, DLQ_NUMS_PER_GROUP, PermName.PERM_WRITE, 0);
            if (null == topicConfig) {
                response.setCode(ResponseCode.SYSTEM_ERROR);
                response.setRemark("topic[" + newTopic + "] not exist");
                return response;
        ...
        if (putMessageResult != null) {
            ...
            case PUT_OK:
                ...
                if (!this.brokerController.getBrokerConfig().isHighSpeedMode()) {
                    ...
                    brokerController.getBrokerStatsManager().incCommercialGroupSndBckTimes(
                    	requestHeader.getGroup(), backTopic, BrokerStatsManager.StatsType.SEND_BACK_SUCCESS.toString(), incValue);

                    brokerController.getBrokerStatsManager().incCommercialGroupSndBckSize(requestHeader.getGroup(), backTopic,
                        BrokerStatsManager.StatsType.SEND_BACK_SUCCESS.toString(), putMessageResult.getAppendMessageResult().getWroteBytes());
                }
            ...
            default:
                // For commercial
                brokerController.getBrokerStatsManager().incCommercialGroupSndBckTimes(
                	requestHeader.getGroup(), msgExt.getTopic(), BrokerStatsManager.StatsType.SEND_BACK_FAILURE.toString(), 1);
                break;
            }
    ...
    private String diskUtil() {
        ...
        String storePathLogis = StorePathConfigHelper.getStorePathConsumeQueue(brokerController.getMessageStoreConfig().getStorePathRootDir());
        double logisRatio = UtilAll.getDiskPartitionSpaceUsedPercent(storePathLogis);

        String storePathIndex = StorePathConfigHelper.getStorePathIndex(brokerController.getMessageStoreConfig().getStorePathRootDir());
    ...
    private RemotingCommand sendMessage(final ChannelHandlerContext ctx, //
        ...
        if (log.isDebugEnabled()) log.debug("receive SendMessage request command, " + request);
        ...
        if (putMessageResult != null) {
            ...
            switch (putMessageResult.getPutMessageStatus()) {
	            ...
	            case SLAVE_NOT_AVAILABLE:
	                ...
	                sendOK = true;
	                break;
	            case CREATE_MAPEDFILE_FAILED:// Failed
	                response.setCode(ResponseCode.SYSTEM_ERROR);
            ...
            if (sendOK) {
                ...
                if (!this.brokerController.getBrokerConfig().isHighSpeedMode()) {
                    // For commercial
                    int incValue = (int) Math.ceil(putMessageResult.getAppendMessageResult().getWroteBytes() / BrokerStatsManager.SIZE_PER_COUNT);
                    brokerController.getBrokerStatsManager().incCommercialTopicSendTimes(requestHeader.getProducerGroup(),
                    					msgInner.getTopic(), BrokerStatsManager.StatsType.SEND_SUCCESS.toString(), incValue);

                    brokerController.getBrokerStatsManager().incCommercialTopicSendSize(requestHeader.getProducerGroup(), msgInner.getTopic(), 
                    			BrokerStatsManager.StatsType.SEND_SUCCESS.toString(), putMessageResult.getAppendMessageResult().getWroteBytes());
                }
                response.setRemark(null);
